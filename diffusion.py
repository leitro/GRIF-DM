...
from blocks import unsqueeze_to, SelfAttention1d
...
class DiffusionModel(nn.Module):
    ...
    def loss(self, x, label):
        ...
        pred_target = self.nn_module(x_t, t_sample, label)
    ...
    # tro add text_feat
    @torch.no_grad()
    def sample(self, text_feat, bsz: int, device: str, num_sampling_timesteps: int | None = None):
        """
        Parameters
        ----------
        ...
        for idx, (scalar_t_start, scalar_t_end) in enumerate(zip(subseq[:-1], subseq[1:])):
            ...
            nn_out = self.nn_module(x, t_start, text_feat) # tro add text_feat
            if self.target_type == "pred_x_0":
                pred_x_0 = nn_out
