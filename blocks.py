...
class SelfAttention1d(nn.Module): # tro add
    ...
    def forward(self, x):
        ...
        k = self.k_lin(x)
        v = self.v_lin(x)
        # batch, length, (heads * features)  256, 21, 768 -> 256*8, 96, 21
        q = rearrange(q, "b l (g c) -> (b g) c l", g=self.num_heads)
        k = rearrange(k, "b l (g c) -> (b g) c l", g=self.num_heads)
        v = rearrange(v, "b l (g c) -> (b g) c l", g=self.num_heads)
        ...
        o = rearrange(o, "(b g) c l -> b l (g c)", g=self.num_heads)
        return x + self.o_lin(o)
...
# tro add
class CrossAttentionLetter(nn.Module):
    def __init__(self, dim, num_heads=8, dropout_prob=0.1):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        self.q_conv = nn.Conv2d(dim, dim, 1, bias=True)
        self.k_proj = nn.Linear(768, dim, bias=True) # hard code BERT 768
        self.v_proj = nn.Linear(768, dim, bias=True) # hard code BERT 768
        self.o_conv = nn.Conv2d(dim, dim, 1, bias=True)
        self.dropout = nn.Dropout(dropout_prob)
    ...
    def forward(self, x, text):
        ...
        k = self.k_proj(text)
        v = self.v_proj(text)
        q = rearrange(q, "b (g c) h w -> (b g) (h w) c", g=self.num_heads) #b,1024,4,4->b*8,16,128
        k = rearrange(k, "b l (g c) -> (b g) l c", g=self.num_heads) #b,3,1024->b*8,3,128
        v = rearrange(v, "b l (g c) -> (b g) l c", g=self.num_heads) #b,3,1024->b*8,3,128
        o = F.scaled_dot_product_attention(q, k, v, attn_mask=None, dropout_p=0.0) 
        o = rearrange(o, "(b g) (h w) c -> b (g c) h w", g=self.num_heads, w=x.shape[-1])
        return x + self.o_conv(o)

# tro add
class CrossAttentionImpression(nn.Module):
    def __init__(self, dim, num_heads=8, dropout_prob=0.1):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
...
class UNet(nn.Module):
    ...
    def __init__(self, in_dim, embed_dim, dim_scales):
        ...
        for idx, (in_c, out_c) in enumerate(zip(
            ...
            self.down_blocks.extend(nn.ModuleList([
                ...
                CrossAttentionImpression(in_c), # tro add
                BasicBlock(in_c, in_c, embed_dim),
                CrossAttentionLetter(in_c), # tro add
        ...
        self.mid_blocks = nn.ModuleList([
            ...
            CrossAttentionImpression(all_dims[-1]), # tro add
            SelfAttention2d(all_dims[-1]),
            CrossAttentionLetter(all_dims[-1]), # tro add
            BasicBlock(all_dims[-1], all_dims[-1], embed_dim),
        ])
    ...
    # tro add label, now it's conditioned diffusion
    def forward(self, x, t, label):
        letter, impression = label
        x = self.init_embed(x)
        t = self.time_embed(t)
        skip_conns = []
        residual = x.clone()
        ...
        for block in self.down_blocks:
            if isinstance(block, BasicBlock):
                x = block(x, t)
                skip_conns.append(x)
            elif isinstance(block, CrossAttentionLetter): # tro add
                x = block(x, letter)
            elif isinstance(block, CrossAttentionImpression): # tro add
                x = block(x, impression)
            else:
                x = block(x)
